...
const action = (row: ResourceData, type: string) => {
...
const delData = async (row: ResourceData | null, multiple: boolean) => {
  tableObject.currentRow = row
...
const enableData = async (row: ResourceData) => {
  const { enableItem } = methods
  await enableItem(row.id)
}

const disableData = async (row: ResourceData) => {
