...
import { getLeaseOrderyApi, getLeaseDetailApi } from '@/api/adminVenueManager/leaseOrder'
...
const { register, tableObject, methods } = useTable<LeaseOrderData>({
  getListApi: getLeaseOrderyApi,
...
setSearchParams({ orderCategory: 'lease' })
...
const action = (row: LeaseOrderData, type: string) => {
...
const exportData = async () => {
  let params = { page: tableObject.currentPage, size: tableObject.pageSize, orderCategory: 'lease' }
...
let printData = reactive({
  total: '0',
  order: {} as LeaseOrderData,
  items: [] as LeaseGoodsDetailData[]
})
...
const print = async (id) => {
  loadStart()
  const res = await getLeaseDetailApi(id)
  loadDone()
  if (res) {
    printData.total = '0'
